...
void Port_Init(const Port_ConfigType* PortConfig)
{
  ...
  for(uint8_least i = 0U; i < countof(Port_ConfigSet.PortNumberOfPortPins); ++i)
  {
    ...
    if(PortConfig->PortNumberOfPortPins[i].Mode == PORT_PIN_MODE_DIO)
    {
      ...
      const uint8 address_of_direction_reg = uint8(port_index + mcal::reg::ddrb);
      ...
      if(PortConfig->PortNumberOfPortPins[i].PinDirection == PORT_PIN_OUT)
      {
        ...
        (PortConfig->PortNumberOfPortPins[i].PinLevelValue == PORT_PIN_LEVEL_HIGH) ? util::dma<uint8,
      ...
      (PortConfig->PortNumberOfPortPins[i].PinDirection == PORT_PIN_IN) ? util::dma<uint8,
...
void Port_SetPinDirection(Port_PinType Pin, Port_PinDirectionType Direction)
{
  ...
  util::dma<uint8, uint8>::bit_clr(address_of_value_reg,     bit_position);
  util::dma<uint8, uint8>::bit_clr(address_of_direction_reg, bit_position);

  (Direction == PORT_PIN_IN) ? util::dma<uint8,
                                         uint8>::bit_set(address_of_value_reg,     bit_position)
                             : util::dma<uint8,
                                         uint8>::bit_set(address_of_direction_reg, bit_position);
}
