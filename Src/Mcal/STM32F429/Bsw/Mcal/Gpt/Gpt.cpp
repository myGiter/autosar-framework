...
void __timer4_irq_handler()
{
  ...
  util::dma<std::uint32_t,
            std::uint16_t>::reg_set(mcal::reg::tim4_sr,
                    0x0000U);
...
void mcal::gpt::init(const config_type*)
{
  ...
  const std::uint32_t aircr_value = util::dma<std::uint32_t,
                                              std::uint32_t>::reg_get(mcal::reg::aircr);
