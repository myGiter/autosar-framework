...
*/
  mcal::gpt::value_type consistent_microsecond_tick()
  {
    ...
    const timer_register_type timer_cnt_1 = util::dma<uint8, uint8>::reg_get(mcal::reg::tcnt0);

    // Read the system tick once.
    const mcal::gpt::value_type sys_tick_1 = system_tick;

    // Do the second read of the timer counter.
    const timer_register_type timer_cnt_2 = util::dma<uint8, uint8>::reg_get(mcal::reg::tcnt0);

    // Perform the consistency check and return the system tick in microseconds.
    return ((timer_cnt_2 >= timer_cnt_1) ? static_cast<mcal::gpt::value_type>(sys_tick_1  | uint8(uint16(uint16(timer_cnt_1) + 1U) >> 1U))
                                         : static_cast<mcal::gpt::value_type>(system_tick | uint8(uint16(uint16(timer_cnt_2) + 1U) >> 1U)));
  }
}
...
void __vector_timer0_ovf_irq(void)
{
  ...
  system_tick += static_cast<uint8>(0x80U);
