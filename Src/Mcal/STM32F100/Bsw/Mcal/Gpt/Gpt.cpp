...
namespace
{
  ...
  mcal::gpt::value_type consistent_microsecond_tick()
  {
    ...
    typedef uint16 timer_register_type;
  ...
  {
    volatile uint32 iser[8U];        // Offset: 0x000  Interrupt Set Enable Register.
             uint32 reserved0[24U];
    volatile uint32 icer[8U];        // Offset: 0x080  Interrupt Clear Enable Register.
             uint32 reserved1[24U];
    volatile uint32 ispr[8U];        // Offset: 0x100  Interrupt Set Pending Register.
             uint32 reserved2[24U];
    volatile uint32 icpr[8U];        // Offset: 0x180  Interrupt Clear Pending Register.
             uint32 reserved3[24U];
    volatile uint32 iabr[8U];        // Offset: 0x200  Interrupt Active bit Register.
             uint32 reserved4[56U];
    volatile uint8  ip[240U];        // Offset: 0x300  Interrupt Priority Register (8Bit wide).
...
void __timer4_irq_handler()
{
  ...
  util::dma<uint32, uint16>::reg_set(mcal::reg::tim4_sr, 0x0000U);
...
void mcal::gpt::init(const config_type*)
{
  ...
  util::dma<uint32,
            uint32>::reg_or(mcal::reg::rcc_apb1enr, 0x00000004UL);
  ...
  const uint32 tmp_prio((0x700UL - (aircr_value & 0x700UL)) >> 0x08);
  const uint32 tmp_pre (0x04UL - tmp_prio);
  const uint32 tmp_sub (0x0FUL >> tmp_prio);

  constexpr uint32 timer4_nvic_irq_preemption_prio(0x0FUL);
  constexpr uint32 timer4_nvic_irq_sub_prio       (0x0FUL);
  ...
  nvic_ptr->ip  [timer4_irq_n] =
    uint32(uint32(  static_cast<uint32>(timer4_nvic_irq_preemption_prio << tmp_pre)
                  | static_cast<uint32>(timer4_nvic_irq_sub_prio & tmp_sub)
                               ) << 0x04U);

  nvic_ptr->iser[timer4_irq_n >> 0x05U] =
    uint32(uint32(1UL) << (timer4_irq_n & 0x1FU));

  // Clear the interrupt request bit.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_sr, 0x0000U);

  // Enable the update interrupt.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_dier, 0x0001U);

  // Set the timer prescaler to 24 resulting in a 1MHz frequency.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_psc, 24U - 1U);

  // Set the auto-reload register for the entire 16-bit period of the timer.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_arr, 0xFFFFU);

  // Set the counter direction to counting-up and enable the counter.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_cr1, 0x0001U);

  // Re-initialize the counter and generate an update of the registers.
  util::dma<uint32,
            uint16>::reg_set(mcal::reg::tim4_egr, 0x0001U);
